...
Public Class frmOrder
    ...
    Sub clear_lvw()
        Try
            With ListView1
                ...
                .Columns.Add("Harga Satuan", 0, HorizontalAlignment.Right)
                .Columns.Add("Discount", 0, HorizontalAlignment.Right)
                .Columns.Add("Subtotal", 0, HorizontalAlignment.Right)
            End With
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Dim m_detail_id As String
        m_detail_id = ""
        If ListView1.CheckedItems.Count > 0 Then
            Try
                For i = 1 To ListView1.Items.Count
                    If ListView1.Items(i - 1).Checked = True Then
                        m_detail_id = m_detail_id + ListView1.Items(i - 1).SubItems.Item(0).Text + ","
                    End If
                Next
                m_detail_id = m_detail_id + "0"
            Catch ex As Exception
                MsgBox("Error Message: " + ex.Message)
                Exit Sub
            End Try
        Else
            MsgBox("Order yang mau di print kosong, silahkan pilih dahulu!", vbInformation)
            Exit Sub
        End If
        ...
        strSQL = "exec RPT_Order_Form '" & m_detail_id & "'"
        Dim DA As New SqlDataAdapter(strSQL, Connection)
        Dim DS As New DataSet

        DA.Fill(DS, "Order_Form")

        Dim strReportPath As String = Application.StartupPath & "\Reports\RPT_Order_Form.rpt"
        ...
        NewMDIChild.Text = "Print Order"
        NewMDIChild.Show()

        Dim repOptions As CrystalDecisions.CrystalReports.Engine.PrintOptions
        cr.Load(strReportPath)
        cr.SetDataSource(DS.Tables("Order_Form"))

        With cr
            repOptions = .PrintOptions
            With repOptions
                .PaperOrientation = CrystalDecisions.Shared.PaperOrientation.Portrait
                .PaperSize = GetPapersizeID(GetSysInit("order_printer_name"), GetSysInit("order_paper_size"))
                .PrinterName = GetSysInit("order_printer_name")
            End With
